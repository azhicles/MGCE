...
Ks = np.arange(1, 21) 
# Array for top-K evaluation. Ks is an array of integers from 1 to 20, representing the top-K items to be recommended.
...
def test_one_user(x):
    ...
    # Randomly sample 99 items from the set of all items minus the items the username has interacted with in training and testing, and add the items the username has interacted with in testing.
    # Items the user has never interacted with.
    # Adds the true test items the user actually interacted with in the test set.
    ...
    def hit_at_k(r, k):
        r = np.array(r)[:k] # top-K recommended items
        if np.sum(r) > 0:
            return 1. # true!
        else:
            return 0.
...
if __name__ == '__main__':
    ...
    model = Model(data_config=config,
                  ...
                  pop_feat=data_generator.popFeaMatrix, d1=4096, d2=300)
