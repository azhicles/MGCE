    ...
    def __init__(self, path, batch_size):
        ...
        with open(pop_file, 'r') as file:
            for line in file.readlines():
                l = line.strip().split(' ')
                item_id = l[0]
                pop_feat = list(map(float, l[1:]))
                self.pop_features[int(item_id)] = pop_feat
            self.popFeaMatrix = [[0.] * d3] * self.n_items
            for item in self.pop_features:
                self.popFeaMatrix[item] = self.pop_features[item]

        self.R = self.R.tocsr()

        # --- Compute average normalized popularity per user ---

        pop_feat = np.array(self.popFeaMatrix).squeeze()  # shape: [n_items]
        min_pop = pop_feat.min()
        max_pop = pop_feat.max()
        pop_norm = (pop_feat - min_pop) / (max_pop - min_pop + 1e-8)  # shape: [n_items], values in [0,1]
        n_users = self.n_users
        user_avg_norm_pop = np.zeros(n_users, dtype=np.float32)
        for uid in range(n_users):
            items = self.train_items.get(uid, [])
            if items:
                user_avg_norm_pop[uid] = np.mean(pop_norm[items])
            else:
                user_avg_norm_pop[uid] = 0.5  # or np.nan or your preferred default
        self.user_avg_norm_pop = user_avg_norm_pop  # shape: [n_users]

        self.coo_R = self.R.tocoo()


    def get_adj_mat(self):
        origin_file = self.path
